generate.votes <- function(true.beta.mat, votes.per.session.vec, 
  VERBOSE=FALSE) {
  ...
  votes.labels <- c("Session.ID", "Winner.ID", "Loser.ID", "Left.Choice.ID",
    "Right.Choice.ID")
  ...
  votes <- array(NA, dim=c(num.votes, length(votes.labels)), 
    dimnames=list(d1=NULL, d2=votes.labels))
  ...
  for (session in (1:num.sessions)) {
    for (vote in (1:votes.per.session.vec[session])) {
      ...
      if (VERBOSE) {
        print(paste("pair:", pair[1], " (", 
          true.beta.mat[session, pair[1]], "),", pair[2] ," (", 
          true.beta.mat[session, pair[2]], ")", sep=""))
      }
      votes[overall.vote.counter, 
        c("Left.Choice.ID", "Right.Choice.ID")] <- pair
      z <- (true.beta.mat[session, pair[1]] - true.beta.mat[session, pair[2]]) +
        rnorm(n=1, mean=0, sd=1)
      if (z > 0) {
        votes[overall.vote.counter, c("Winner.ID", "Loser.ID")] <- c(pair[1], 
          pair[2])
      } else {
        votes[overall.vote.counter, c("Winner.ID", "Loser.ID")] <- c(pair[2], 
          pair[1])
      }
