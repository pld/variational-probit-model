...
convert.votes.to.sparse.design.matrix <- function(set.of.votes, VERBOSE=FALSE) {
  ...
  # check input
  if (any(set.of.votes[, "Left.Choice.ID"]==set.of.votes[,"Right.Choice.ID"]))
    stop("ERROR: Some objects seem to compete against themself.  That's not possible.")
  ...
  if (VERBOSE && length(beta.parameters.identified.labels)) { 
  ...
  beta.parameters.not.identified.object.labels <- ifelse(length(beta.parameters.not.identified.labels), get.objects(beta.parameters.not.identified.labels, VERBOSE=FALSE), c(""))
  if (VERBOSE) { 
    print("head(beta.parameters.not.identified.object.labels):")
    print(head(beta.parameters.not.identified.object.labels))
  }
  beta.parameters.identified.object.labels <- get.objects(beta.parameters.identified.labels, VERBOSE=FALSE)
  if (VERBOSE) { 
    print("head(beta.parameters.identified.object.labels):")
    print(head(beta.parameters.identified.object.labels)) 
  }
